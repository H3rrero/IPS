...
public class VentanaPagoCuota extends JDialog {
	 ...
	 * Create the dialog.
	 */
	public VentanaPagoCuota(VentanaPrincipal vp) {
		gbd = new GestorBaseDatos();
		setTitle("Realizar pago mediante cuota de socio");
	...
	private JButton getBtBuscar() {
		if (btBuscar == null) {
			...
			btBuscar.setIcon(new ImageIcon(VentanaPagoCuota.class
	...
	private JTable getTableImpagos() {
		if (tableImpagos == null) {
			String[] nombreCols = { "DNI Socio", "Instalacion", "Horario",
					"Inicio", "Fin", "Cod. Reserva", "Importe", "Cobro" };
			modeloTabla = new ModeloNoEditable(nombreCols, 0);
			tableImpagos = new JTable(modeloTabla);
			tableImpagos.setDefaultRenderer(Object.class,
					new RendererSubstance());
	...
	private void añadirFilas(String cadena) throws ClassNotFoundException,
			SQLException {
		gbd = new GestorBaseDatos();
		...
		for (Reserva r : reservas) {
			if (r.getDNI().equals(cadena)) {
				if (r.isCobro() == false && r.isCancelada() == false) {
					...
					nuevaFila[6] = importeAPagar(r.getHoraInicio(),
							r.getHoraFin(),
							getNombreInstalacion(r.getId_instalacion()))
							+ " €";
	...
	public String getNombreInstalacion(int cad) throws ClassNotFoundException,
			SQLException {
	...
	private JButton getBtPagar() {
		if (btPagar == null) {
			btPagar = new JButton("Incluir en cuota de socio");
			btPagar.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					int fila = tableImpagos.getSelectedRow();
					String dni = String.valueOf(tableImpagos
							.getValueAt(fila, 0));
					String instalacion = String.valueOf(tableImpagos
							.getValueAt(fila, 1));
					String horario = String.valueOf(tableImpagos.getValueAt(
							fila, 2));
					String inicio = String.valueOf(tableImpagos.getValueAt(
							fila, 3));
					String fin = String.valueOf(tableImpagos
							.getValueAt(fila, 4));
					String codReserva = String.valueOf(tableImpagos.getValueAt(
							fila, 5));
					String imp = String.valueOf(tableImpagos
							.getValueAt(fila, 6));
					String[] importes = imp.split(" ");

					generarRecibo("RECIBOCOD" + codReserva, dni, instalacion,
							horario, inicio, fin, codReserva, imp);
					double importe;
					try {
						importe = leerImporte()
								+ Double.parseDouble(importes[0]);
						incluirEnCuotaSocio(dni, importe, codReserva);
					...
					JOptionPane.showMessageDialog(null,
							"Se ha generado el recibo con ID: RECIBOCOD"
									+ codReserva, "Recibo pagado",
							JOptionPane.PLAIN_MESSAGE);
					dispose();
				}
			});
			btPagar.setEnabled(false);
			btPagar.setBounds(258, 259, 181, 37);
		}
		return btPagar;
	}
	...
	private JButton getBtCancelar() {
		if (btCancelar == null) {
			...
			btCancelar.setBounds(449, 259, 89, 37);
		}
		return btCancelar;
	}

	public int getPrecioInstalacion(String nombre)
	...
	public int importeAPagar(int horaInicio, int horaFin, String nombre)
			throws ClassNotFoundException, SQLException {
	...
	private void generarRecibo(String fileName, String dni, String instalacion,
			String horario, String inicio, String fin, String codReserva,
			String importe) {
		...
		try {
			...
			pw.println(instalacion + " " + "el dia: " + horario
					+ " en el horario: " + inicio + ":00 h -" + fin + ":00 h"
					+ " con CODIGO DE RESERVA: " + codReserva);
			pw.println("El importe de: " + importe);
			pw.println("EL IMPORTE SE HA INCLUIDO EN LA CUOTA DEL SOCIO CORRECTAMENTE");
			pw.println("----------------------------<<<PAGADO>>>----------------------------");
	...
	private void incluirEnCuotaSocio(String dni, double importe,
			String codReserva) throws ClassNotFoundException, SQLException {
		Conexion con = new Conexion();
		con.conectar();
		PreparedStatement pst = null;
		try {
			pst = con.getCon().prepareStatement(
					"UPDATE Socios SET Importe_Mensual='" + importe
							+ "' WHERE DNI_Socio = '" + dni + "'");
			pst.executeUpdate();
			pst = con.getCon().prepareStatement(
					"UPDATE Reservas SET Cobro='" + true
	...
	private boolean comprobarSocioBaseDatos(String dni)
			throws ClassNotFoundException, SQLException {
	...
	public double leerImporte() throws ClassNotFoundException, SQLException {
		Conexion con = new Conexion();
		con.conectar();
		java.sql.Statement st = con.getCon().createStatement();
		ResultSet rs = st
				.executeQuery("SELECT * FROM Socios WHERE DNI_Socio= '"
						+ txDNI.getText().toString() + "'");
		while (rs.next()) {
			return Double.parseDouble(rs.getString("Importe_Mensual"));
		}
		rs.clearWarnings();
		st.close();
		con.desconectar();
	...
	public boolean comprobarCentroBaseDatos(String dni)
			throws ClassNotFoundException, SQLException {
